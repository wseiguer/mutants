 ...
 */
package meli.exam.mutants.utils.suffixtree;
...
class Node {
     ...
     * INCREMENT).
     * 
     * Originally it was a List<Integer> but it took too much memory, changing it to
     * int[] take less memory because indexes are stored using native types.
     */
    private int[] data;
    /**
    ...
    /**
     * The suffix link as described in Ukkonen's paper. if str is the string denoted
     * by the path from the root to this, this.suffix is the node denoted by the
     * path that corresponds to str without the first char.
     */
    private Node suffix;
    /**
     * The total number of <em>different</em> results that are stored in this node
     * and in underlying ones (i.e. nodes that can be reached through paths starting
     * from <tt>this</tt>.
     * 
    ...
    /**
     * Returns the first <tt>numElements</tt> elements from the ones associated to
     * this node.
     *
     ...
     * children. Should be called after having called computeAndCacheCount.
     * 
     * @throws IllegalStateException when this method is called without having
     *                               called computeAndCacheCount first
     * @see Node#computeAndCacheCount()
